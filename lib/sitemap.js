...
/**
 * Build all brand-related sitemap URL entries:
 * 1) /brand/{brand_slug}
 * 2) /category/brand/{category_slug}/{brand_slug} for main categories with products
 */
export async function getBrandSitemapEntries(baseUrl) {
  const Brand = (await import("@/models/ecom_brand_info")).default;
  const Product = (await import("@/models/product")).default;
  await dbConnect();

  const [brands, mainCategories, allCategories] = await Promise.all([
    Brand.find(
      {
        status: "Active",
  ...
  for (const cat of allCategories) {
    const parentKey = String(cat.parentid || "none");
    if (!childrenByParent.has(parentKey)) childrenByParent.set(parentKey, []);
    childrenByParent.get(parentKey).push(cat._id.toString());
  }

  function getTreeIds(rootId) {
    const ids = [rootId];
    const queue = [rootId];
    while (queue.length) {
  ...
  const entries = brands.map((b) => ({
    loc: `${baseUrl}/brand/${b.brand_slug}`,
  ...
  for (const category of mainCategories) {
    const treeIds = getTreeIds(category._id.toString());
    const brandIds = await Product.distinct("brand", {
      status: "Active",
      brand: { $exists: true, $ne: "" },
      $or: [
        { category: { $in: treeIds } },
        { sub_category: { $in: treeIds } },
      ],
    });

    for (const brandId of brandIds) {
      const brand = brandById.get(String(brandId));
      if (!brand?.brand_slug) continue;

      entries.push({
        loc: `${baseUrl}/category/brand/${category.category_slug}/${brand.brand_slug}`,
        lastmod: brand.updatedAt || category.updatedAt || new Date(),
        changefreq: "weekly",
        priority: "0.65",
      });
    }
