...
export function sanitizeMetaKeywords(value) {
  if (value == null) return "";
  const text = String(value).trim();
  if (!text) return "";
  const lower = text.toLowerCase();
  if (["none", "null", "undefined", "n/a", "na", "-"].includes(lower)) return "";
  return text;
}
...
export function buildBlogPostingSchema(baseUrl, blog, options = {}) {
  ...
  const description =
    (blog.meta_description &&
      ...
      String(blog.meta_description).trim()) ||
    stripHtml(blog.description || "").slice(0, 300);
