...
export default function CategoryPage() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const apiData = await categoryRes.json();

      setCategoryData({
        ...apiData,
        categoryTree: apiData.category,
        allCategoryIds: apiData.allCategoryIds,
        main_category: apiData.main_category,
      });

      if (apiData.products?.length > 0) {
        const prices = apiData.products.map((p) => p.special_price);
      ...
      setPagination({
        currentPage: 1,
        totalPages: Math.ceil(apiData.products.length / itemsPerPage),
        hasNext: apiData.products.length > itemsPerPage,
        hasPrev: false,
        totalProducts: apiData.products.length,
      });
      setNofound(apiData.products.length === 0);
    } catch (error) {
  ...
  const fetchFilteredProducts = useCallback(
    async (md5, pageNum = 1) => {
      console.log("fetchFilteredProducts called! md5:", md5);
      try {
        setLoading(true);

        const query = new URLSearchParams();
        query.set("sub_category_new", md5);
  ...
  useEffect(() => {
    const md5 = categoryData.category?.[0]?.parentid_new;
    if (md5) {
  ...
  const handlePageChange = (page) => {
    const md5 = categoryData.category?.[0]?.parentid_new;
    if (page >= 1 && page <= pagination.totalPages && md5) {
      fetchFilteredProducts(md5, page);
            ...
            <Image
              src={
                categoryData.banners[
                  ...
                  ? categoryData.banners[currentCategoryBannerIndex]
                      .banner_image
                ...
                {Object.values(filterGroups).map((group) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="flex justify-between items-center w-full group bg-gray-300 p-1"
                    >
                          ...
                          <li key={filter._id} className="flex items-center">
                            <label className="flex items-center space-x-2 w-full cursor-pointer hover:bg-gray-50 rounded p-2 transition-colors">
                              <input
                                ...
                                onChange={() =>
                                  handleFilterChange("filters", filter._id)
                                }
        ...
        {!nofound && products.length > 0 ? (
          <>
            ...
            <div
              ...
              className="products-container flex-1"
            >
              {products.length > 0 ? (
                <>
