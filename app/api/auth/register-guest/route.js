...
export async function POST(req) {
  try {
    ...
    if (password) {
      ...
      hashedPassword = await bcrypt.hash(guestName, 10); // 🔥 guest1, guest2...
    }


    const countEmail = await GuestUser.countDocuments();
    const guestEmail = `guestmail${countEmail + 1}@gmail.com`;

    const newUser = new GuestUser({
        name: name || guestName,
        mobile,
        email: email || guestEmail,        // ✅ always allow empty
        password: hashedPassword || "", // ✅ allow empty
    });
