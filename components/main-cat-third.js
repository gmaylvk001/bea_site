...
export default function CategoryMainPage({ categorySlug = "large-appliance" }) {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // STEP 1: Include categorySlug so backend resolves categories
      if (categorySlug) query.set("categorySlug", categorySlug);

      // STEP 2: Apply filters - CORRECTED PARAMETER NAMES
      if (selectedFilters?.categories?.length > 0) {
        query.set("categoryIds", selectedFilters.categories.join(","));
      }

      if (selectedFilters?.brands?.length > 0) {
        query.set("brands", selectedFilters.brands.join(",")); // CORRECTED: "brands" not "brandIds"
      }
      ...
      query.set("limit", itemsPerPage || 12);
      ...
      setTotalProducts(data.total || data.pagination?.totalProducts || 0);
