...
export function buildModelPopupContent({ snapshot, config } = {}) {
  const cfg = config || getDefaultSmartLeadConfig();
  const p = productFromSnapshot(snapshot);
  const label = p.name || "this product";
...
export function buildComparisonPopupContent({ snapshot, config } = {}) {
  ...
  const currentCat =
    ...
    snapshot?.lastViewedProduct?.categoryId ||
    "";
  ...
  return applyOverrides(
    ...
    "COMPARISON",
    cfg,
    contentVars({ categoryName, product: productFromSnapshot(snapshot) })
  );
}
...
export function buildPremiumPopupContent({ snapshot, config } = {}) {
  const cfg = config || getDefaultSmartLeadConfig();
  const p = productFromSnapshot(snapshot);
  const label = p.name || "this premium product";
