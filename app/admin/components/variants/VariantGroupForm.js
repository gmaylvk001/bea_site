...
export default function VariantGroupForm({ groupId = null }) {
  ...
  const addAttribute = () => {
    ...
    setAttributes((prev) => [...prev, { name: nextName, type: "text", valuesMeta: [] }]);
  ...
  return (
    <div className="py-6 space-y-5 w-full max-w-full min-w-0">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 w-full max-w-full min-w-0 overflow-hidden">
        ...
        {selectedProducts.length === 0 ? (
          ...
          <div className="w-full max-w-full min-w-0">
            ...
            <div className="w-full max-w-full min-w-0 overflow-x-auto overflow-y-visible border border-gray-200 rounded bg-white">
              <div
                className="inline-grid align-top"
                style={{
                  gridTemplateColumns: `160px repeat(${selectedProducts.length}, 200px)`,
                  width: `${160 + selectedProducts.length * 200}px`,
                }}
              >
                <div className="sticky left-0 z-20 border-b border-r border-gray-200 bg-gray-50 px-3 py-2 font-medium text-gray-700">
                  Variant
                </div>
                {selectedProducts.map((product, index) => (
                  <div
                    key={`head-${product._id}`}
                    className="border-b border-r border-gray-200 bg-gray-50 px-3 py-2 align-top"
                  >
                ...
                {attributes.map((attr, attrIndex) => (
                  <div key={`row-${attrIndex}`} className="contents">
                    <div className="sticky left-0 z-10 border-b border-r border-gray-200 bg-gray-50 px-3 py-2">
                      <input
                        value={attr.name}
                    ...
                    {selectedProducts.map((product) => (
                      <div
                        ...
                        className="border-b border-r border-gray-200 px-3 py-2 align-top"
                      >
                        {attr.type === "color" ? (
                          <div className="flex items-center gap-2">
                            <input
                              type="color"
                              value={getValueMeta(attr, product.values?.[attr.name] || "").colorHex || "#000000"}
                              onChange={(e) => {
                                const currentValue = product.values?.[attr.name] || "";
                                if (currentValue) {
                                  updateValueMeta(attrIndex, currentValue, { colorHex: e.target.value });
                                }
                              }}
                              className="h-8 w-8 flex-shrink-0 border rounded"
                            />
                            <input
                              value={product.values?.[attr.name] || ""}
        ...
        <button
          ...
          className="mt-3 inline-flex items-center gap-2 text-sm text-blue-700"
        >
          <FaPlus /> Add Variant
        </button>
      </div>
