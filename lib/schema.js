...
export function buildProductSchema(baseUrl, product, options = {}) {
  ...
  const reviewCount =
    options.reviewCount != null ? Number(options.reviewCount) : reviews.length;
  const avgRating =
    options.avgRating != null
  ...
  if (reviewCount > 0 && avgRating > 0) {
    schema.aggregateRating = {
      ...
      ratingValue: String(Number(avgRating.toFixed(1))),
  ...
  if (reviews.length > 0) {
    schema.review = reviews.slice(0, 10).map((r) => {
      const authorName =
        ...
        r.author ||
        "Customer";
      const ratingValue = r.reviews_rating || r.rating || r.reviewRating;
      ...
      return {
        ...
        ...(r.created_date || r.datePublished
          ? {
              datePublished: toSchemaDate(r.created_date || r.datePublished),
            }
          : {}),
      };
    });
